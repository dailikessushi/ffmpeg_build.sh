...
sudo apt-get -y install autoconf automake build-essential \
   ...
   libaribb24-dev texi2html zlib1g-dev gcc make diffutils
cpu_core=24
enable_h264=1
enable_h265=1
enable_aom=1
...
repo_yasm="https://github.com/yasm/yasm.git"
repo_x264="https://git.videolan.org/git/x264.git"
repo_x265="https://github.com/videolan/x265.git"
repo_aom="https://aomedia.googlesource.com/aom"
repo_svt="https://gitlab.com/AOMediaCodec/SVT-AV1.git"
repo_aac="https://github.com/mstorsjo/fdk-aac"
repo_opus="https://github.com/xiph/opus.git"
repo_libvpx="https://chromium.googlesource.com/webm/libvpx.git"
repo_libass="https://github.com/libass/libass.git"
repo_ffmpeg="https://github.com/FFmpeg/FFmpeg"
repo_amf="https://github.com/GPUOpen-LibrariesAndSDKs/AMF.git"

#url_nasm="https://www.nasm.us/pub/nasm/releasebuilds/2.14.02/nasm-2.14.02.tar.bz2"
url_nasm="https://www.nasm.us/pub/nasm/releasebuilds/2.16.03/nasm-2.16.03.tar.bz2"
...
url_ogg="https://ftp.osuosl.org/pub/xiph/releases/ogg/libogg-1.3.5.tar.gz"
#url_theora="https://ftp.osuosl.org/pub/xiph/releases/theora/libtheora-1.1.1.tar.bz2"
url_theora="https://ftp.osuosl.org/pub/xiph/releases/theora/libtheora-1.2.0.tar.gz"
...
if [ $enable_aom -eq 1 ]; then
    ...
    mkdir -p ../aom_build
    cd ../aom_build
    cmake -G "Unix Makefiles" -DCMAKE_INSTALL_PREFIX="$prefix_dir" -DENABLE_TESTS=OFF -DENABLE_NASM=on ../aom
    make -j${cpu_core}
    if [ $? -ne 0 ]; then
        print_error "make libaom" && exit 1
    fi
    make install
    make clean
    enable_option="${enable_option} --enable-libaom"
fi

if [ $enable_svt -eq 1 ]; then
    echo "---------- build libsvtav1  ----------"
    cd $src_dir
    run_git $repo_svt ""
    mkdir -p build
    cd build
    cmake -G "Unix Makefiles" -DCMAKE_INSTALL_PREFIX="$prefix_dir" -DCMAKE_BUILD_TYPE=Release -DBUILD_DEC=OFF -DBUILD_SHARED_LIBS=OFF ..
    make -j${cpu_core}
    if [ $? -ne 0 ]; then
        print_error "make libsvtav1" && exit 1
    fi
    make install
    make clean
    enable_option="${enable_option} --enable-libsvtav1"
fi
...
if [ $enable_amf -eq 1 ]; then
    echo "---------- build amf ----------"
    sudo mkdir -p /usr/local/include/AMF
    cd $src_dir
    sudo git clone $repo_amf AMF
    sudo cp -r AMF/amf/public/include/* /usr/local/include/AMF/
    #run_git $repo_amf "--depth 1"
    #autoreconf -fiv
    enable_option="${enable_option} --enable-amf"
fi


if [ $enable_va -eq 1 ]; then
    echo "---------- build VA ----------"
    #run_git $repo_amf "--depth 1"
    #autoreconf -fiv
    enable_option="${enable_option} --enable-vaapi"
fi
...
#echo "export PATH=\$PATH:/usr/local/ffmpeg_build/bin" >> /home/dai/.bashrc
